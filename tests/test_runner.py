...
@pytest.mark.asyncio
async def test_run_task_returns_result_per_case() -> None:
    task = _make_task(3)
    provider = _RecordingProvider()

    results = await run_task(task, provider, model="m")
...
@pytest.mark.asyncio
async def test_run_task_renders_prompt_template() -> None:
    task = _make_task(1)
    provider = _RecordingProvider()

    await run_task(task, provider, model="m")

    assert provider.inputs == ["Extract from: input 0"]


@pytest.mark.asyncio
async def test_run_task_respects_concurrency_limit() -> None:
    task = _make_task(10)
    provider = _RecordingProvider(delay=0.01)

    await run_task(task, provider, model="m", concurrency=3)

    assert provider.max_in_flight <= 3


@pytest.mark.asyncio
async def test_run_task_skips_failed_case_and_keeps_survivors() -> None:
    task = _make_task(3)
